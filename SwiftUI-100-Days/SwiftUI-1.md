...
### Grid Layout in SwiftUI (LazyHGrid & LazyVGrid):
There are two Lazy grid layout in SwiftUI, `LazyHGrid(column:)` and `LazyVGrid(row:)`. Both of them required a `[GridItem]` array as param of column/row.
We can use `ScrollView` to place them inside.
